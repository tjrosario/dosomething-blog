    ...
    if ($post_type == '') {
     ...
     if (has_post_thumbnail()) {
            $output .= '<article id="' . get_the_ID() . '" class="mk-blog-spotlight-item spotlight-'.$item_id.' mk-isotop-item '.$mk_column_css.' ' . $post_type . '-post-type">' . "\n";
                ...
                $output .= '    <img alt="' . get_the_title() . '" title="' . get_the_title() . '" src="' . $image_output_src . '" itemprop="image" /></a>';
                ...
                $output .=                do_shortcode( '[mk_button dimension="outline" corner_style="pointed" outline_skin="custom" outline_active_color="#ffffff" outline_hover_color="#444" bg_color="#00c8d7" btn_hover_bg="#dd3333" text_color="light" size="medium" align="center" url="'.get_permalink().'"]'.__('READ MORE', 'mk_framework').'[/mk_button]' );
                $output .= '        </div>';
                $output .= '    </div>';
                $output .= '</div>';
