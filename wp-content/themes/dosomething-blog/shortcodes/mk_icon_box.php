...
if ( $style == "simple_minimal" ) {
    if ( $circled == 'true' ) {
        ...
        $output .= !empty( $read_more_url ) ? '<a href="'.$read_more_url.'"><i class="'.$icon.$animation_css.' circled-icon mk-main-ico" style="'.$border_css.'color:'.$icon_color.';background-color:'.$icon_circle_color.'"></i></a>' : '<i class="'.$icon.$animation_css.' circled-icon mk-main-ico" style="'.$border_css.'color:'.$icon_color.';background-color:'.$icon_circle_color.'"></i>';
        $output .= !empty( $read_more_url ) ? '<a href="'.$read_more_url.'">'.$title.'</a>' : $title;
        $output .= '</h4>';
    }   else {
        $output .= '<h4 style="font-size:'.$text_size.'px;font-weight:'.$font_weight.';"><i style="color:'.$icon_color.'" class="'.$icon.$animation_css.' mk-main-ico"></i>';
        $output .= !empty( $read_more_url ) ? '<a href="'.$read_more_url.'">'.$title.'</a>' : $title;
        $output .= '</h4>';
    }
    ...
    if ( $read_more_txt ) {
        $output .= '<a class="icon-box-readmore" href="'.$read_more_url.'">'.$read_more_txt.'<i class="mk-icon-caret-right"></i></a>';
    }
