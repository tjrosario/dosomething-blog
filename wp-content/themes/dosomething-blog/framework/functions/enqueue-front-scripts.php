...
function mk_theme_enqueue_scripts() {
	if ( !is_admin() && !( in_array( $GLOBALS['pagenow'], array( 'wp-login.php', 'wp-register.php' ) ) )) {
		...
		wp_enqueue_script( 'theme-lib-min', THEME_DIR_URI .'/dist/js/lib.js', array( 'jquery' ), $theme_data['Version'], true );

		wp_enqueue_script( 'theme-app-min', THEME_DIR_URI .'/dist/js/app.js', array( 'jquery' ), $theme_data['Version'], true );
		...
		if ( $mk_options['special_fonts_type_1'] == 'google' && !empty( $mk_options['special_fonts_list_1'] ) ) {
			...
			wp_enqueue_style( 'google-font-api-special-1', 'http'.((is_ssl())? 's' : '').'://fonts.googleapis.com/css?family=' .$mk_options['special_fonts_list_1'].':300italic,400italic,600italic,700italic,800italic,400,300,800,700,600'.$subset_1 , false, false, 'all' );
		}
		...
		if ( $mk_options['special_fonts_type_2'] == 'google' && !empty( $mk_options['special_fonts_list_2'] ) ) {
			...
			wp_enqueue_style( 'google-font-api-special-2', 'http'.((is_ssl())? 's' : '').'://fonts.googleapis.com/css?family=' .$mk_options['special_fonts_list_2'].':300italic,400italic,600italic,700italic,800italic,400,300,800,700,600'.$subset_2  , false, false, 'all' );
