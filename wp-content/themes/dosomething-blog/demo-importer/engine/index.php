...
if ($max_execution_time < 400 || $max_input_time < 300 || mk_nummerize(WP_MEMORY_LIMIT) < 67108864 || mk_nummerize($upload_max_filesize) < 10485760) {
    
    echo '<div class="error settings-error">';
    
    echo '<br><strong>Please resolve these issues before instlling any template.</strong>';
    echo '<ol>';
    if ($max_execution_time < 400) {
    ...
    if ($max_input_time < 300)
        echo '<li><strong>Maximum Input Time (max_input_time) : </strong>' . $max_input_time . ' seconds. <span style="color:red"> Recommended max_input_time should be at least 300 Seconds.</span></li>';
    if (mk_nummerize(WP_MEMORY_LIMIT) < 67108864) {
        echo '<li><strong>WordPress Memory Limit (WP_MEMORY_LIMIT) : </strong>' . WP_MEMORY_LIMIT . ' <span style="color:red"> Recommended memory limit should be at least 64MB. Please refer to : <a target="_blank" href="http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP">Increasing memory allocated to PHP</a> for more information</span></li>';
    }
  ...
  (function($) {
    "use strict";
        $('.mk-import-content-btn').click(function(e){
           ...
           var $import_true = confirm('Are you sure to import dummy content? We highly encourage you to do this action in a WordPress fresh installtion!');
            
            if($import_true == false) return;

            $('.import_message').html('<div class="updated settings-error"><div class="import-content-loading">Please be patient while template is being imported. Depeding from the template you are intended to install, this process may take between 3-10 minutes.</div></div>');
