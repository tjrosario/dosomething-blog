...
foreach ( $images as $attach_id ) {
    ...
    $item .= '<li>';
    $item .= '<img alt="" src="' . $image_src[0] .'" />';
    $item .= '</li>'. "\n\n";

}

$backgroud_image = !empty( $bg_image ) ? 'background-image:url('.$bg_image.'); ' : '';
$output .= '<div class="mk-fullwidth-slideshow mk-script-call mk-flexslider '.$enable_3d.$el_class.'" data-speedFactor="'.$speed_factor.'" style="padding:'.$padding.'px 0;'. $backgroud_image.'background-color:'.$bg_color.'; background-position:'.$bg_position.';background-attachment:'.$attachment.';border:1px solid '.$border_color.';border-left:none;border-right:none;" data-animation="'.$effect.'" data-easing="swing" data-direction="horizontal" data-smoothHeight="'.$smooth_height.'" data-animationSpeed="'.$animation_speed.'" data-slideshowSpeed="'.$slideshow_speed.'" data-pauseOnHover="'.$pause_on_hover.'" data-controlNav="false" data-directionNav="'.$direction_nav.'" data-isCarousel="false">';
