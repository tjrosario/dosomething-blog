...
$custom_sidebars           = $pages = $posts = $categories = $edge_posts = $testimonials = $clients = $news_posts = $portfolio_posts = $flexslider = $employees = $pricing = $animated_columns = $authors = $tab_slider = array();
$target_arr = array(
...
if(is_admin()) {
$tab_slider_entries = get_posts('post_type=tab_slider&orderby=title&numberposts=30&order=ASC');
...
$animated_columns_entries = get_posts('post_type=animated-columns&orderby=title&numberposts=30&order=ASC');
...
$pricing_entries = get_posts('post_type=pricing&orderby=title&numberposts=30&order=ASC');
if ($pricing_entries != null && !empty($pricing_entries)) {
    foreach ($pricing_entries as $key => $entry) {
        $pricing[$entry->ID] = $entry->post_title;
    }
}
$employees_entries = get_posts('post_type=employees&orderby=title&numberposts=30&order=ASC');
...
$flexslider_entries = get_posts('post_type=slideshow&orderby=title&numberposts=30&order=ASC');
...
$edge_entries = get_posts('post_type=edge&orderby=title&numberposts=30&order=ASC');
if ($edge_entries != null && !empty($edge_entries)) {
    foreach ($edge_entries as $key => $entry) {
        $edge_posts[$entry->ID] = $entry->post_title;
    }
}
$portfolio_entries = get_posts('post_type=portfolio&orderby=title&numberposts=30&order=ASC');
foreach ($portfolio_entries as $key => $entry) {
    $portfolio_posts[$entry->ID] = $entry->post_title;
}
$posts_entries = get_posts('post_type=post&orderby=title&numberposts=30&order=ASC');
foreach ($posts_entries as $key => $entry) {
    $posts[$entry->ID] = $entry->post_title;
}
$news_entries = get_posts('post_type=news&orderby=title&numberposts=30&order=ASC');
foreach ($news_entries as $key => $entry) {
    $news_posts[$entry->ID] = $entry->post_title;
}
$clients_entries = get_posts('post_type=clients&orderby=title&numberposts=30&order=ASC');
if ($clients_entries != null && !empty($clients_entries)) {
    foreach ($clients_entries as $key => $entry) {
        $clients[$entry->ID] = $entry->post_title;
    }
}
$testimonials_entries = get_posts('post_type=testimonial&orderby=title&numberposts=30&order=ASC');
